...
from classes.utils.utils import euler
...
class MeshViewer(object):
    ...
    def __init__(self,
        ...
        camera_pose[:3, :3] = euler(camera_dict['ca_rotate'], 'xyz')#[80, 0, 30]
        camera_pose[:3, 3] = np.array(camera_dict['ca_translate'])#[1.2, -2.3, 0.5]
        ...
        if self.offscreen:
            ...
            # light=SpotLight(color=np.ones(3), intensity=3.0, innerConeAngle=0.1, outerConeAngle=0.7853981633974483)
            
            light=DirectionalLight(color=np.ones(3), intensity=light_dict['li_intens'])
            light_pose = np.eye(4)
            light_pose[:3, :3] = euler(light_dict['li_rotate'], 'xyz')
            light_pose[:3, 3] = light_dict['li_translate']
